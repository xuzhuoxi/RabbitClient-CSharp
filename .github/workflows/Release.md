...
本库通过 `ProjectReference` 依赖旁路目录中的 `Infra-CSharp`。工作流会把两个仓库检出为兄弟目录，与本地 `JLGameStudios/Infra-CSharp` 与 `JLGameStudios/RabbitClient-CSharp` 的布局一致。
...
可参考模板 `notes/release/ReleaseNotes.md`。该模板本身 **不会** 被 workflow 读取。也可用仓库内 Cursor Skill `generate-note` 根据**本地**提交范围生成该文件。

Release 正文规则：

- 若该文件存在：先写入文件全文，再追加 GitHub 自动生成的 notes。
...
| 类型 | 文件名示例 |
| --- | --- |
| Debug DLL 压缩包 | `RabbitClient-CSharp_v1.0.3_debug_netstandard2.0.zip` |
| Release DLL 压缩包 | `RabbitClient-CSharp_v1.0.3_release_netstandard2.0.zip` |
| 校验和 | `SHA256SUMS.txt` |

<!-- 暂不启用：NuGet 包 `JLGames.RabbitClient.1.0.3.nupkg`、符号包 `JLGames.RabbitClient.1.0.3.snupkg` -->

每个 zip 内包含：`RabbitClient.dll`、`RabbitClient.pdb`、`RabbitClient.deps.json`、`Infra-CSharp.dll`（及若存在的 `Infra-CSharp.pdb`）、`LICENSE`、`README.md`、`README_EN.md`。Debug 包来自 `bin/Debug`，Release 包来自 `bin/Release`。
